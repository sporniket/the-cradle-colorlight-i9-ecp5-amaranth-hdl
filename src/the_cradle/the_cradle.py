...
class TheCradle(Elaboratable):
    ...
    def elaborate(self, platform: Platform) -> Module:
        ...
        domDviLink = ClockDomain("dviLink", local = True) # for the shift registers receiving TMDS encoded data
        m.d.comb += domDviLink.clk.eq(mainPll.clkout0)

        domTheCradle = ClockDomain("theCradle", local = True) # save for later, unused now
        m.d.comb += domTheCradle.clk.eq(mainPll.clkout1)

        domPixel = ClockDomain("pixel", local = True) # for the TMDS encoder and the pixel source.
        m.d.comb += domPixel.clk.eq(mainPll.clkout2, local = True)

        domCpuBase = ClockDomain("cpuBase", local = True) # base for clocking the external CPU, unused now
        m.d.comb += domCpuBase.clk.eq(mainPll.clkout3, local = True)

        ### Pixel sequencer -> hsync clock
        m.submodules.pixelSequencer = pixelSequencer = DomainRenamer("pixel")(Sequencer(pixelSequence))
        ...
        m.submodules.scanlineSequencer = scanlineSequencer = DomainRenamer("hsync")(Sequencer(scanlineSequence))
        ...
        domHsyncActive = ClockDomain("hsyncActive", local = True) # in effect, serves to clock a counter of displayed line, reset at each vbl.
        m.d.comb += [
            domHsyncActive.clk.eq(hsyncActive),
            domHsyncActive.rst.eq(vsync)
        ]

        ### The pixel source
        # TODO : step 0 : solid blue-ish screen
